	...
	<script type="text/javascript">
            ...
            $.ajax({
                ...
                success:function (reg) {
                    if (reg.state==1){
                        ...
                        $("#province-list").empty();
                        $("#province-list").append('<option value="0">----- 请选择 -----</option>')
                        $.each(reg.result,function (i,n) {
                            $("#province-list").append('<option value="'+n.code+'">'+n.name+'</option>')
                        })
                    } else {
                        alert(reg.msg);
                    }
            ...
            $.ajax({
                ...
                success:function (reg) {
                    if (reg.state==1){
                        ...
                        $.each(reg.result,function (i,n) {
                            $("#city-list").append('<option value="'+n.code+'">'+n.name+'</option>')
                        })
                    } else {
                        alert(reg.msg);
                    }
        ...
        function selectArea() {
            $.ajax({
                ...
                dataType:"json",
                data:{
                    parent:$("#city-list").val()
                },
                success:function (reg) {
                    if (reg.state==1){

                        $("#area-list").empty();
                        $("#area-list").append('<option value="0">----- 请选择 -----</option>')
                        $.each(reg.result,function (i,n) {
                            $("#area-list").append('<option value="'+n.code+'">'+n.name+'</option>')
                        })
                    } else {
                        alert(reg.msg);
                    }
						...
						<form id="form-add-new-address" class="form-horizontal" role="form">
							...
							<div class="address_content" data-toggle="distpicker">
								<div class="form-group">
									...
									<div class="col-md-3">
										<select id="city-list" name="cityCode" class="form-control" ></select>
									</div>
								</div>
								<div class="form-group">
							...
							<div class="form-group">
								<label class="col-md-2 control-label"><a href="web/address.jsp">返回</a>&nbsp;&nbsp;&nbsp;</label>
								<div class="col-sm-10">
